...
- `npm run env:push-gh` – upload local dev secrets to GitHub repo secrets
- `npm run env:pull-gh` – download secrets from GitHub and restore locally
- `npm run env:export` – create a local secrets archive (USB / cloud backup)

## Syncing dev environment between machines
...
```bash
npm run env:export
# copies bundle to your machine — transfer it securely, then on the new machine:
